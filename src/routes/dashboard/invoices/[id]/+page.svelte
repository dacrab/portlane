...
<div class="no-print mb-6 flex items-center gap-3">
	<a href="/dashboard/invoices" class="text-sm" style="color:var(--color-text-faint)">← Invoices</a>
...
<div class="mx-auto max-w-2xl rounded-xl p-10" style="background:var(--color-bg-elevated);border:1px solid var(--color-border)">
	...
	<div class="mb-10 flex items-start justify-between gap-6">
		<div>
			<div class="flex items-center gap-2 mb-4">
				...
				<span class="text-sm font-semibold" style="color:var(--color-text-heading)">Portlane</span>
			</div>
			<p class="text-3xl font-bold tracking-tight" style="color:var(--color-text-heading)">Invoice</p>
			<p class="mt-1 text-xs font-mono" style="color:var(--color-text-faint)">{inv.id.slice(0, 8).toUpperCase()}</p>
		</div>
		<div class="text-right">
			...
			<p class="mt-3 text-xs" style="color:var(--color-text-faint)">Issued {today}</p>
			{#if inv.due_date}
	...
	<div class="mb-10 grid grid-cols-2 gap-8">
		<div>
			<p class="mb-1 text-xs font-semibold uppercase tracking-widest" style="color:var(--color-text-faint)">From</p>
			<p class="text-sm font-semibold" style="color:var(--color-text-heading)">{freelancerName}</p>
		</div>
		<div>
			<p class="mb-1 text-xs font-semibold uppercase tracking-widest" style="color:var(--color-text-faint)">To</p>
			<p class="text-sm font-semibold" style="color:var(--color-text-heading)">{clientName}</p>
		</div>
	</div>
	...
	<div class="mb-10 overflow-hidden rounded-lg" style="border:1px solid var(--color-border)">
		...
		<div class="grid px-5 py-4" style="grid-template-columns:1fr auto">
			<div>
				<p class="text-sm font-medium" style="color:var(--color-text-heading)">{project?.name ?? 'Project'}</p>
				{#if project?.description}
					<p class="mt-0.5 text-xs" style="color:var(--color-text-faint)">{project.description}</p>
				{/if}
			</div>
			<p class="text-sm font-semibold" style="color:var(--color-text-heading)">
	...
	<div class="flex justify-end">
		<div class="w-48">
			<div class="flex justify-between py-2 text-sm" style="border-top:2px solid var(--color-border)">
				<span class="font-semibold" style="color:var(--color-text-heading)">Total</span>
				<span class="font-bold text-base" style="color:var(--color-text-heading)">
					{inv.currency.toUpperCase()} {(inv.amount_cents / 100).toLocaleString('en-US', { minimumFractionDigits: 2 })}
				</span>
			</div>
		</div>
	...
	<!-- Footer -->
	<p class="mt-10 text-center text-xs" style="color:var(--color-text-faint)">
