...
type TimeEntry = (typeof data.timeEntries)[number]

let { data }: { data: PageData } = $props()
let project = $derived(data.project)
let milestones = $derived(data.milestones)
let timeEntries = $derived(data.timeEntries)
...
let timeDesc = $state('')
let noteBody = $state(untrack(() => note))
let projectStatus = $state(untrack(() => project.status))

const totalMilestones = $derived(milestoneTotal(milestones))
const doneMilestones = $derived(milestoneDone(milestones))
const progress = $derived(milestoneProgress(milestones))
const totalMinutes = $derived(
	timeEntries.reduce((s: number, e: TimeEntry) => s + e.minutes, 0),
)
const totalHours = $derived((totalMinutes / 60).toFixed(1))

function copyPortalLink() {
	navigator.clipboard.writeText(
		`${window.location.origin}/portal?project=${project.id}`,
	)
...
function exportTimeCSV() {
	...
	a.download = `${project.name}-time-entries.csv`
	a.click()
...
<div class="space-y-6">
	...
	<div class="flex items-start gap-3">
		...
		<div class="flex-1 min-w-0">
			...
			<div class="mt-2 flex flex-wrap items-center gap-4 text-xs text-faint">
				{#if project.dueDate}
					<span>Due {fmtDateLong(project.dueDate)}</span>
				{/if}
	...
	<div class="grid gap-5 lg:grid-cols-[1fr_22rem]">
		...
		<div class="space-y-5">
					...
					<div class="mb-4 overflow-hidden rounded-lg" style="border:1px solid var(--color-border-subtle)">
						{#each timeEntries as e, i}
							<div class="flex items-center gap-3 px-4 py-3" style="{i > 0 ? 'border-top:1px solid var(--color-border-subtle)' : ''}">
								...
								<span class="text-xs text-faint">{fmtDate(e.loggedAt)}</span>
							</div>
						{/each}
					</div>
				{/if}
			...
			<!-- Comments -->
			<div class="card">
				<SectionHeader title="Comments" icon={IconChatTextRegular} count={comments.length || undefined} />
						...
						{#each comments as c}
							<div class="flex items-start gap-3">
								...
								<div class="rounded-lg px-3 py-2.5 max-w-[85%]" style="background:var(--color-bg)">
									<p class="mb-0.5 text-xs font-semibold text-faint">{c.name ?? 'Unknown'}</p>
						...
						{#each clients as c}
							<div class="flex items-center gap-2.5 rounded-lg px-3 py-2" style="background:var(--color-bg)">
								...
								<div class="min-w-0 flex-1">
									<p class="text-sm font-medium truncate text-body">{c.name ?? '—'}</p>
								</div>
