...
async function seed() {
	...
	await admin.from('comments').insert([
		{
			...
			body: 'Wireframes are ready for review. Let me know your thoughts!',
		},
		{
			project_id: brandProject.id,
			author_id: client1Id,
			body: 'Looks great! Can we explore a darker color palette?',
		},
		{
			project_id: mobileProject.id,
			author_id: freelancerId,
			body: 'Prototype will be ready by end of week.',
		},
	])
	...
	console.log(
		`   Portal URL: http://localhost:3000/portal?project=${brandProject.id}`,
	)
}
