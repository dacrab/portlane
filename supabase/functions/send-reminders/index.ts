...
serve(async () => {
	...
	const fromEmail =
		Deno.env.get('REMINDER_FROM_EMAIL') ?? 'noreply@portlane.app'
	...
	const { data: invoices } = await supabase
		.from('invoices')
		.select(
			'id, amount_cents, due_date, status, client_id, project_id, last_reminder_sent_at',
		)
	...
	if (invoices && resendKey) {
		for (const inv of invoices) {
			...
			const lastSent = inv.last_reminder_sent_at
				? new Date(inv.last_reminder_sent_at)
				: null
			...
			if (inv.status === 'overdue' && (!lastSent || lastSent < weekAgo)) {
				...
				inv.due_date === in3DaysStr &&
				inv.status === 'sent' &&
				(!lastSent || lastSent < dayAgo)
			) {
				await send(
					resendKey,
					fromEmail,
		...
		for (const c of comments) {
			...
			const notifyId =
				author.role === 'freelancer'
					? await getClient(supabase, c.project_id)
					: proj.freelancer_id
...
// Returns the first client linked to the project (projects currently link a
// single client for notification purposes).
